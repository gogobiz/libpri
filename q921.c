...
static void arinc_q921_dump_pri(struct q921_link *link, char direction_tag);
...
static void arinc_q921_setstate(struct q921_link *link, int newstate)
{
	...
	if (ctrl->debug & PRI_DEBUG_Q921_STATE) {
		...
		if (ctrl->debug && link->arinc_state != newstate) {
...
static int arinc_q921_transmit(struct pri *ctrl, q921_h *h, int len) 
{
	int res;

	if (ctrl->debug)
		pri_message(ctrl, "Got to arinc_q921_transmit for a frame of length %d", len);
	...
	if (ctrl->debug & (PRI_DEBUG_Q921_DUMP | PRI_DEBUG_Q921_RAW))
		arinc_q921_dump(ctrl, h, len, ctrl->debug & PRI_DEBUG_Q921_RAW, 1);
	...
	if (ctrl->debug)
		pri_message(ctrl, "At end of arinc_q921_transmit and we believe we have a return result of %d\n", res);
...
static int q921_transmit(struct pri *ctrl, q921_h *h, int len) 
{
	...
	if (ctrl->debug)
		pri_message(ctrl, "At end of q921_transmit and we believe we have a return result of %d\n", res);
...
static void arinc_q921_send_dm(struct q921_link *link, int fbit)
{
	...
	arinc_q921_transmit(ctrl, &h, 4);
}

static void arinc_q921_send_disc(struct q921_link *link, int pbit)
{
	q921_h h;
	struct pri *ctrl;
	...
	if (ctrl->debug & PRI_DEBUG_Q921_STATE) {
		pri_message(ctrl, "ARINC TEI=%d Sending DISC\n", link->tei);
	}
	arinc_q921_transmit(ctrl, &h, 4);
}
...
static int arinc_q921_ack_packet(struct q921_link *link, int num)
{
	struct q921_frame *f;
	...
	for (prev = NULL, f = link->arinc_tx_queue; f; prev = f, f = f->next) {
		...
		if (f->h.h.sapi != 2 ) {
                        if (ctrl->debug) pri_message(ctrl, "ARINC tx_queue SKIPPING ack for a SAPI (%d) packet\n", f->h.h.sapi);
			break;
		}		
		if (f->status != Q921_TX_FRAME_SENT) {
			if (ctrl->debug) pri_message(ctrl, "ARINC inside q921 ack packet, but frame status indicates we should break\n");
			break;
		}
		if (f->h.n_s == num) {
			if (ctrl->debug) pri_message(ctrl, "ARINC inside q921 ack packet and found frame match for %d\n", num);
...
static void arinc_stop_t203(struct q921_link *link)
{
	...
	if (link->arinc_t203_timer) {
		...
		if (ctrl->debug & PRI_DEBUG_Q921_DUMP)
			pri_message(ctrl, "-- ARINC T203 requested to stop when not started\n");
	}
}
...
static int arinc_q921_send_queued_iframes(struct q921_link *link)
{
	...
	for (; f; f = f->next) {
		...
		*/
		...
		f->h.ft = 0;
		f->h.p_f = 0;
		arinc_q921_transmit(ctrl, (q921_h *) (&f->h), f->len);
	...
	for (f = link->tx_queue; f; f = f->next) {
		...
		*/
		if (f->status != Q921_TX_FRAME_SENT) {
			/* This frame needs to be sent. */
			if (ctrl->debug)
				pri_message(ctrl, "In q921_send_queued_iframe and found the frame we need to send");
			break;
		}
	}
	if (!f) {
...
static void arinc_q921_reject(struct q921_link *link, int pf)
{
	q921_h h;
	struct pri *ctrl;
	...
	h.s.n_r = link->arinc__v_r;	/* Where to start retransmission N(R) */
	h.s.p_f = pf;	
	switch (ctrl->localtype) {
	case PRI_NETWORK:
		h.h.c_r = 0;
...
static void arinc_q921_rr(struct q921_link *link, int pbit, int cmd)
{
        q921_h h;
        struct pri *ctrl;
...
int q921_transmit_iframe(struct q921_link *link, void *buf, int len, int cr)
{
	...
	case Q921_MULTI_FRAME_ESTABLISHED:
		...
		if (ctrl->debug)
			pri_message(ctrl, "We are in state Q921_MULTI_FRAME_ESTABLISHED in q921_transmit_frame and attempting to send frame\n");
		...
		if (f) {
			...
			if (link->state != Q921_MULTI_FRAME_ESTABLISHED) {
				if (ctrl->debug)
					pri_message(ctrl, "Something weird just happened and our state changed mid-transmit to %s\n", q921_state2str(link->state));
			...
			if (link->peer_rx_busy) {
				if (ctrl->debug)
					pri_message(ctrl, "Peer link is busy, so queueing iframe - yikes.\n");
			...
			if (!q921_send_queued_iframes(link)) {
				if (ctrl->debug)
					pri_message(ctrl, "Bad. We attempted to send a q921 queued frame and we instead are queued with window shut!\n");
				/*
	...
	ctrl = link->ctrl;
        ...
        case Q921_MULTI_FRAME_ESTABLISHED:
			...
			f->h.h.sapi = ARINC_SAPI_EQMTCTRL; // TODO: Send a SAPI 2 frame
                        ...
                        if (!arinc_q921_send_queued_iframes(link)) {
                                /*
                                 * No frames sent even though we just put a frame on the queue.
                                 *
                                 * Special debug message/test here because we want to say what
				...
				if (ctrl->debug) pri_message(ctrl, "ARINC Sent queued iframe\n");
			}
                } else {
...
static void arinc_q921_dump_pri_by_h(struct pri *ctrl, char direction_tag, q921_h *h);

void arinc_q921_dump(struct pri *ctrl, q921_h *h, int len, int showraw, int txrx)
{
	int x;
	const char *type;
	char direction_tag;
	...
	pri_message(ctrl, "\n");
	arinc_q921_dump_pri_by_h(ctrl, direction_tag, h);
...
static void arinc_q921_dump_pri(struct q921_link *link, char direction_tag)
{
	...
	pri_message(ctrl, "%c TEI: %d State %d(%s)\n",
		direction_tag, link->tei, link->arinc_state, q921_state2str(link->arinc_state));
	pri_message(ctrl, "%c V(A)=%d, V(S)=%d, V(R)=%d\n",
		direction_tag, link->arinc__v_a, link->arinc__v_s, link->arinc__v_r);
	pri_message(ctrl, "%c K=%d, RC=%d, l3_initiated=%d, reject_except=%d, ack_pend=%d\n",
		direction_tag, ctrl->timers[PRI_TIMER_K], link->arinc_RC, link->arinc_l3_initiated,
		link->arinc_reject_exception, link->arinc_acknowledge_pending);
	pri_message(ctrl, "%c T200_id=%d, N200=%d, T203_id=%d\n",
		direction_tag, link->arinc_t200_timer, ctrl->timers[PRI_TIMER_N200], link->arinc_t203_timer);
}
...
static void arinc_q921_dump_pri_by_h(struct pri *ctrl, char direction_tag, q921_h *h)
{
	...
	if (link) {
		arinc_q921_dump_pri(link, direction_tag);
	} else {
		pri_message(ctrl, "%c Link not found for this frame.\n", direction_tag);
	}
}
...
static pri_event *arinc_q921_ua_rx(struct q921_link *link, q921_h *h)
{
	struct pri *ctrl;
	pri_event *res = NULL;
	enum Q931_DL_EVENT delay_q931_dl_event;
...
static void arinc_q921_enquiry_response(struct q921_link *link)
{
        struct pri *ctrl;
...
static int arinc_n_r_is_valid(struct q921_link *link, int n_r)
{
	int x;

	for (x = link->arinc__v_a; x != n_r && x != link->arinc__v_s; Q921_INC(x)) {
	}
	if (x != n_r) {
		return 0;
	} else {
...
static pri_event *arinc_q921_rr_rx(struct q921_link *link, q921_h *h)
{
        pri_event *res = NULL;
        struct pri *ctrl;
        ...
        case Q921_MULTI_FRAME_ESTABLISHED:
                ...
                if (is_command(ctrl, h)) {
                  ...
                  if (!arinc_n_r_is_valid(link, h->s.n_r)) {
			if (ctrl->debug) pri_message(ctrl, "ARINC in %s with arinc_n_r_is_valid = false!\n", __func__);
                       arinc_n_r_error_recovery(link);
                       arinc_q921_setstate(link, Q921_AWAITING_ESTABLISHMENT);                } else {
			if (ctrl->debug) pri_message(ctrl, "ARINC in %s with arinc_n_r_is_valid = true.\n", __func__);
...
static pri_event *arinc_q921_rej_rx(struct q921_link *link, q921_h *h)
{
	pri_event *res = NULL;
	struct pri *ctrl;
	...
	case Q921_MULTI_FRAME_ESTABLISHED:
		...
		if (is_command(ctrl, h)) {
			...
			if (h->s.p_f) {
				// Todo- do I want to kill this for SAPI2?
				arinc_q921_mdl_error(link, 'A');
			}
		}

		/* We want to allow retransmissions, but probably not allow anything to jack with the state of the link, if it's SAPI2 anyway */
		if (!arinc_n_r_is_valid(link, h->s.n_r)) {
			arinc_n_r_error_recovery(link);
			arinc_q921_setstate(link, Q921_AWAITING_ESTABLISHMENT);
		} else {
	...
	case Q921_AWAITING_RELEASE:
		...
		break;
	default:
		pri_error(ctrl, "ARINC Don't know what to do with REJ in state %d(%s)\n",
			link->arinc_state, q921_state2str(link->arinc_state));
		break;
	}
...
static pri_event *q921_iframe_rx(struct q921_link *link, q921_h *h, int len)
{
	...
	case Q921_MULTI_FRAME_ESTABLISHED:
		...
		if (link->own_rx_busy) {
			 ...
			 */
			if ((ctrl->debug & PRI_DEBUG_Q931_DUMP) && h->h.sapi != 2 ) { // TODO: fixup ARINC debug for DEFINITION
			//if ((ctrl->debug & PRI_DEBUG_Q931_DUMP)) { // TODO: fixup ARINC debug for DEFINITION				
...
static pri_event *arinc_q921_iframe_rx(struct q921_link *link, q921_h *h, int len)
{
	struct pri *ctrl;
	pri_event *eres = NULL;
	int res = 0;
	int arinc_delay_q931_receive;
	...
	case Q921_MULTI_FRAME_ESTABLISHED:
		...
		if (link->arinc_own_rx_busy) {
			...
			if (ctrl->debug) pri_message(ctrl, "ARINC in %s and incrementing arinc__v_r counter to %d\n", __func__, link->arinc__v_r);
			link->arinc_reject_exception = 0;
			 ...
			 */
			if ((ctrl->debug & PRI_DEBUG_Q931_DUMP)) { // TODO: fixup ARINC debug for DEFINITION
				arinc_q931_dump(ctrl, h->h.tei, (arinc_q931_h *) h->i.data, len - 4, 0);
			}
			...
			if (link->arinc_reject_exception) {
				if (ctrl->debug) pri_message(ctrl, "ARINC Reject exception was true\n");
				if (h->i.p_f) {
					if (ctrl->debug) pri_message(ctrl, "ARINC Reject exception with PF bit of %d\n now sending q921_rr", h->i.p_f);					
					arinc_q921_rr(link, 1, 0);
					link->arinc_acknowledge_pending = 0;
				}
			} else {
				if (ctrl->debug) pri_message(ctrl, "ARINC in %s setting arinc_reject_exception to 1", __func__);
		...
		if (arinc_delay_q931_receive) {
			if (ctrl->debug) pri_message(ctrl, "ARINC in method %s with delayed receive = TRUE. Now running q931 receive\n", __func__);
			/* Q.921 has finished processing the frame so we can give it to Q.931 now. */					
			res = q931_receive(link, (q931_h *) h->i.data, len - 4);
...
static pri_event *arinc_q921_rnr_rx(struct q921_link *link, q921_h *h)
{
	pri_event *res = NULL;
	struct pri *ctrl;
...
static void arinc_q921_acknowledge_pending_check(struct q921_link *link)
{
	if (link->arinc_acknowledge_pending) {
		link->arinc_acknowledge_pending = 0;
		arinc_q921_rr(link, 0, 0);
	}
}
...
static pri_event *__arinc_q921_receive_qualified(struct q921_link *link, q921_h *h, int len)
{
	int res;
	pri_event *ev = NULL;
	struct pri *ctrl;
	...
	case 0:
	case 2:
		ev = arinc_q921_iframe_rx(link, h, len);
		break;
	case 1:
		switch ((h->s.x0 << 2) | h->s.ss) {
		case 0x00:
				ev = arinc_q921_rr_rx(link, h);
			break;
 		case 0x01:
				ev = arinc_q921_rnr_rx(link, h);
			break;
 		case 0x02:
				ev = arinc_q921_rej_rx(link, h);
			break;
		default:
			pri_error(ctrl,
	...
	case 3:
		...
		case 0x00:
			/* UI-frame */
			if (ctrl->debug & PRI_DEBUG_Q931_DUMP)
					arinc_q931_dump(ctrl, h->h.tei, (arinc_q931_h *) h->u.data, len - 3, 0);
	...
	arinc_q921_statemachine_check(link);
...
/* TODO: Gallient effort
static pri_event *__arinc_q921_receive_qualified(struct q921_link *link, q921_h *h, int len)
{
	pri_event *ev = NULL;
	//int old;
	struct q921_link *newLink;

	pri_message(link->ctrl, "arinc receive qualified\n");

	Q921_INIT(newLink, h);
	//memcpy(&link, newLink, sizeof(link));
	newLink->sapi = ARINC_SAPI_EQMTCTRL; // TODO: MAKE LESS HACKY
	// NOT: q921_send_ua(link, h->u.p_f);
	ev =  __q921_receive_qualified(newLink, h, len);
	//pri_message(link->ctrl, "SAPI IS NOW: %d\n", link->sapi);
	//pri_message(link->ctrl, "SAPI IS NOW: %d\n", link->sapi);
	free(newLink);	
	return ev;
}
...
static pri_event *__q921_receive(struct pri *ctrl, q921_h *h, int len)
{
	...
	// if ( ( ctrl->debug & (PRI_DEBUG_Q921_DUMP | PRI_DEBUG_Q921_RAW) ) && !h->h.sapi == ARINC_SAPI_EQMTCTRL ) {
	if ( ( ctrl->debug & (PRI_DEBUG_Q921_DUMP | PRI_DEBUG_Q921_RAW) ) ) {
	...
	if (BRI_TE_PTMP(ctrl)) {
		...
		&& (h->h.tei == ctrl->link.tei || h->h.tei == Q921_TEI_GROUP)
		&& ctrl->switchtype == PRI_SWITCH_ARINC) {
			//pri_message(ctrl, "Header SAPI and Link SAPI are different, but its a Equipment control message\n"):
			pri_message(ctrl, "SAPI %d on SAPI %d link\n", h->h.sapi, ctrl->link.sapi);
			ev = __arinc_q921_receive_qualified(&ctrl->link, h, len);
	} else 	{
		ev = NULL;
	}
