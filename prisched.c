...
static int arinc_schedule_grow(struct pri *ctrl)
{
	unsigned num_slots;
	struct arinc_pri_sched *timers;

	/* Determine how many slots in the new timer table. */
	if (ctrl->arinc_sched.num_slots) {
		if (SCHED_EVENTS_MAX <= ctrl->arinc_sched.num_slots) {
			/* Cannot grow the timer table any more. */
			return -1;
		}
		num_slots = ctrl->arinc_sched.num_slots * 2;
		if (SCHED_EVENTS_MAX < num_slots) {
			num_slots = SCHED_EVENTS_MAX;
		}
	} else {
	...
	/* Get and initialize the new timer table. */
	timers = calloc(num_slots, sizeof(struct arinc_pri_sched));
	if (!timers) {
		/* Could not get a new timer table. */
		return -1;
	}
	if (ctrl->arinc_sched.timer) {
		/* Copy over the old timer table. */
		memcpy(timers, ctrl->arinc_sched.timer,
			ctrl->arinc_sched.num_slots * sizeof(struct arinc_pri_sched));
		free(ctrl->arinc_sched.timer);
	} else {
		/* Creating the timer pool. */
		pool_id += SCHED_EVENTS_MAX;
		if (pool_id < SCHED_EVENTS_MAX
		...
		ctrl->arinc_sched.first_id = pool_id;
	}

	/* Put the new timer table in place. */
	ctrl->arinc_sched.timer = timers;
	ctrl->arinc_sched.num_slots = num_slots;
	return 0;
}
...
unsigned arinc_schedule_event(struct pri *ctrl, int ms, void (*function)(void *data), void *data)
{
	...
	max_used = ctrl->arinc_sched.max_used;
	for (x = 0; x < max_used; ++x) {
		if (!ctrl->arinc_sched.timer[x].callback) {
			break;
		}
	}
	if (x == ctrl->arinc_sched.num_slots && arinc_schedule_grow(ctrl)) {
		pri_error(ctrl, "No more room in scheduler\n");
		return 0;
	}
	if (ctrl->arinc_sched.max_used <= x) {
		ctrl->arinc_sched.max_used = x + 1;
	}
	if (x >= maxsched) {
		maxsched = x + 1;
	}
	...
	ctrl->arinc_sched.timer[x].when = tv;
	ctrl->arinc_sched.timer[x].callback = function;
	ctrl->arinc_sched.timer[x].data = data;
	return ctrl->arinc_sched.first_id + x;
}
...
static pri_event *__arinc_schedule_run(struct pri *ctrl, struct timeval *tv)
{
	unsigned x;
	unsigned max_used;
	void (*callback)(void *);
	...
	for (x = 0; x < max_used; ++x) {
		if (ctrl->arinc_sched.timer[x].callback
			&& ((ctrl->arinc_sched.timer[x].when.tv_sec < tv->tv_sec)
			|| ((ctrl->arinc_sched.timer[x].when.tv_sec == tv->tv_sec)
			&& (ctrl->arinc_sched.timer[x].when.tv_usec <= tv->tv_usec)))) {
			/* This timer has expired. */
			ctrl->schedev = 0;
			callback = ctrl->arinc_sched.timer[x].callback;
			data = ctrl->arinc_sched.timer[x].data;
			ctrl->arinc_sched.timer[x].callback = NULL;
			callback(data);
			if (ctrl->arinc_schedev) {
				return &ctrl->ev;
			}
		}
	}
...
pri_event *arinc_schedule_run(struct pri *ctrl)
{
	struct timeval tv;

	gettimeofday(&tv, NULL);
	return __arinc_schedule_run(ctrl, &tv);
}
...
void arinc_schedule_del(struct pri *ctrl, unsigned id)
{
	...
	if (ctrl->arinc_sched.first_id <= id
		&& id <= ctrl->arinc_sched.first_id + (SCHED_EVENTS_MAX - 1)) {
		ctrl->arinc_sched.timer[id - ctrl->arinc_sched.first_id].callback = NULL;
		return;
	}
	if (ctrl->nfas) {
		/* Try to find the timer on another D channel. */
		for (nfas = PRI_NFAS_MASTER(ctrl); nfas; nfas = nfas->slave) {
			if (nfas->arinc_sched.first_id <= id
				&& id <= nfas->arinc_sched.first_id + (SCHED_EVENTS_MAX - 1)) {
				nfas->arinc_sched.timer[id - nfas->arinc_sched.first_id].callback = NULL;
				return;
			}
		}
	}
	pri_error(ctrl,
		"Asked to delete sched id 0x%08x??? first_id=0x%08x, num_slots=0x%08x\n", id,
		ctrl->arinc_sched.first_id, ctrl->arinc_sched.num_slots);
}
...
int arinc_schedule_check(struct pri *ctrl, unsigned id, void (*function)(void *data), void *data)
{
	...
	if (ctrl->arinc_sched.first_id <= id
		&& id <= ctrl->arinc_sched.first_id + (SCHED_EVENTS_MAX - 1)) {
		return ctrl->arinc_sched.timer[id - ctrl->arinc_sched.first_id].callback == function
			&& ctrl->arinc_sched.timer[id - ctrl->arinc_sched.first_id].data == data;
	}
	if (ctrl->nfas) {
		/* Try to find the timer on another D channel. */
		for (nfas = PRI_NFAS_MASTER(ctrl); nfas; nfas = nfas->slave) {
			if (nfas->arinc_sched.first_id <= id
				&& id <= nfas->arinc_sched.first_id + (SCHED_EVENTS_MAX - 1)) {
				return nfas->arinc_sched.timer[id - nfas->arinc_sched.first_id].callback == function
					&& nfas->arinc_sched.timer[id - nfas->arinc_sched.first_id].data == data;
			}
		}
	}
	pri_error(ctrl,
		"Asked to check arinc_sched id 0x%08x??? first_id=0x%08x, num_slots=0x%08x\n", id,
		ctrl->arinc_sched.first_id, ctrl->arinc_sched.num_slots);
	return 0;
}
